    ...
    public function write(bool $devMode, InstallationManager $installationManager)
    {
        ...
        if ($this->dumpVersions) {
            ...
            if ($installedVersionsClass !== false) {
                ...
                \BracketSpace\Notification\bbPress\Dependencies\Composer\InstalledVersions::reload($versions);
            }
        }
    }
...
REGEX;
        if (is_string($installedVersionsData) && Preg::isMatch($pattern, trim($installedVersionsData))) {
            \BracketSpace\Notification\bbPress\Dependencies\Composer\InstalledVersions::reload(eval('?>'.Preg::replace('{=>\s*+__DIR__\s*+\.\s*+([\'"])}', '=> '.var_export(dirname($path), true).' . $1', $installedVersionsData)));
