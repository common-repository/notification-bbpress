...
namespace BracketSpace\Notification\bbPress\Dependencies\Composer\Installer;
...
use BracketSpace\Notification\bbPress\Dependencies\Composer\DependencyResolver\Operation\MarkAliasInstalledOperation;
use BracketSpace\Notification\bbPress\Dependencies\Composer\DependencyResolver\Operation\MarkAliasUninstalledOperation;
use BracketSpace\Notification\bbPress\Dependencies\Composer\Downloader\FileDownloader;
...
class InstallationManager
{
    ...
    private function executeBatch(InstalledRepositoryInterface $repo, array $operations, array $cleanupPromises, bool $devMode, bool $runScripts, array $allOperations): void
    {
        ...
        foreach ($operations as $index => $operation) {
            ...
            if (!$promise instanceof PromiseInterface) {
                $promise = \BracketSpace\Notification\bbPress\Dependencies\React\Promise\resolve(null);
            }
    ...
    private function runCleanup(array $cleanupPromises): void
    {
        ...
        foreach ($cleanupPromises as $cleanup) {
            $promises[] = new \BracketSpace\Notification\bbPress\Dependencies\React\Promise\Promise(static function ($resolve) use ($cleanup): void {
                $promise = $cleanup();
                if (!$promise instanceof PromiseInterface) {
                    $resolve(null);
                } else {
