...
namespace BracketSpace\Notification\bbPress\Dependencies\Composer\Console;

use BracketSpace\Notification\bbPress\Dependencies\Composer\Installer;
use BracketSpace\Notification\bbPress\Dependencies\Composer\IO\NullIO;
...
use BracketSpace\Notification\bbPress\Dependencies\Seld\JsonLint\ParsingException;
use BracketSpace\Notification\bbPress\Dependencies\Composer\Command;
...
use BracketSpace\Notification\bbPress\Dependencies\Composer\IO\ConsoleIO;
use BracketSpace\Notification\bbPress\Dependencies\Composer\Json\JsonValidationException;
use BracketSpace\Notification\bbPress\Dependencies\Composer\Util\ErrorHandler;
use BracketSpace\Notification\bbPress\Dependencies\Composer\Util\HttpDownloader;
use BracketSpace\Notification\bbPress\Dependencies\Composer\EventDispatcher\ScriptExecutionException;
use BracketSpace\Notification\bbPress\Dependencies\Composer\Exception\NoSslException;
use BracketSpace\Notification\bbPress\Dependencies\Composer\XdebugHandler\XdebugHandler;
use BracketSpace\Notification\bbPress\Dependencies\Symfony\Component\Process\Exception\ProcessTimedOutException;
use BracketSpace\Notification\bbPress\Dependencies\Composer\Util\Http\ProxyManager;
...
';
    ...
    public function __construct(string $name = 'BracketSpace\Notification\bbPress\Dependencies\Composer', string $version = '')
    {
            ...
            if (is_file($file) && Filesystem::isReadable($file) && is_array($composer = json_decode(file_get_contents($file), true))) {
                if (isset($composer['scripts']) && is_array($composer['scripts'])) {
                    foreach ($composer['scripts'] as $script => $dummy) {
                        if (!defined('BracketSpace\Notification\bbPress\Dependencies\Composer\Script\ScriptEvents::'.str_replace('-', '_', strtoupper($script)))) {
                            if ($this->has($script)) {
     ...
     * @return \BracketSpace\Notification\bbPress\Dependencies\Symfony\Component\Console\Command\Command[]
     */
    ...
    private function getPluginCommands(): array
    {
        ...
        if (null !== $composer) {
            ...
            foreach ($pm->getPluginCapabilities('BracketSpace\Notification\bbPress\Dependencies\Composer\Plugin\Capability\CommandProvider', ['composer' => $composer, 'io' => $this->io]) as $capability) {
