...
namespace BracketSpace\Notification\bbPress\Dependencies\Composer\Downloader;
...
use BracketSpace\Notification\bbPress\Dependencies\Composer\Exception\IrrecoverableDownloadException;
use BracketSpace\Notification\bbPress\Dependencies\Composer\Package\Comparer\Comparer;
...
use BracketSpace\Notification\bbPress\Dependencies\Composer\Util\Url as UrlUtil;
...
class FileDownloader implements DownloaderInterface, ChangeReportInterface
{
    ...
    public function download(PackageInterface $package, string $path, ?PackageInterface $prevPackage = null, bool $output = true): PromiseInterface
    {
        ...
        $download = function () use ($output, $cacheKeyGenerator, $package, $fileName, &$urls, &$accept, &$reject) {
            ...
            if ($this->cache !== null && ($checksum === null || $checksum === '' || $checksum === $this->cache->sha1($cacheKey)) && $this->cache->copyTo($cacheKey, $fileName)) {
                ...
                $result = \BracketSpace\Notification\bbPress\Dependencies\React\Promise\resolve($fileName);
            } else {
                if ($output) {
    ...
    {
        return \BracketSpace\Notification\bbPress\Dependencies\React\Promise\resolve(null);
    }
