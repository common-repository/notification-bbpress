...
class Filesystem
{
    ...
    public function removeDirectoryAsync(string $directory)
    {
        $edgeCaseResult = $this->removeEdgeCases($directory);
        if ($edgeCaseResult !== null) {
            return \BracketSpace\Notification\bbPress\Dependencies\React\Promise\resolve($edgeCaseResult);
        }
        ...
        return $promise->then(function ($process) use ($directory) {
            ...
            if ($process->isSuccessful()) {
                if (!is_dir($directory)) {
                    return \BracketSpace\Notification\bbPress\Dependencies\React\Promise\resolve(true);
                }
            }

            return \BracketSpace\Notification\bbPress\Dependencies\React\Promise\resolve($this->removeDirectoryPhp($directory));
        });
    }
